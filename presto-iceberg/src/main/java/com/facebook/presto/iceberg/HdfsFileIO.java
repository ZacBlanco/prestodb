        ...
        implements FileIO
{
    ...
    {
        return new HdfsInputFile(new Path(path), environment, context, Optional.empty());
    }
    ...
    public InputFile newInputFile(ManifestFile manifest)
    {
        checkArgument(
                ...
                manifest.path());
        return new HdfsCachedInputFile(new HdfsInputFile(new Path(manifest.path()), environment, context, Optional.of(manifest.length())), new ManifestFileCacheKey(manifest.path()), manifestFileCache);
    }
