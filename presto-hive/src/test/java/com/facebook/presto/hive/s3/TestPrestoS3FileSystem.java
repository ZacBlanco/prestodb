...
public class TestPrestoS3FileSystem
{
            ...
            throws IOException, URISyntaxException
    {
        try (PrestoS3FileSystem fs = new PrestoS3FileSystem()) {
            ...
            assertEquals(s3.initiateMultipartRequests, 1);
            assertEquals(s3.uploadPartRequests, 2);
            assertEquals(s3.completePartUploadRequests, 1);
        }
    }
    ...
    private void assertDirEmpty(java.nio.file.Path path)
            throws IOException
    {
        try (Stream<java.nio.file.Path> dirIter = Files.walk(path)) {
            List<java.nio.file.Path> items = dirIter.collect(Collectors.toList());
            assertEquals(items.size(), 1L);
    ...
    @Test
    public void testRoundTripWrite()
            throws IOException, URISyntaxException
    {
