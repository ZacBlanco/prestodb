...
_______________
* Fix query statistics so that `planningTime` and `finishingTime` are no longer added to `executionTime`. `executionTime` is now the true execution time — how long it took the query to run the compute. It can be used to measure the efficiency of the workers without added planning time or the time spent on final steps such as partition registration. `#27691 <https://github.com/prestodb/presto/pull/27691>`_
* Fix a bug in the optimizer that could cause validation errors for queries with cascading projections above a cross join. `#27568 <https://github.com/prestodb/presto/pull/27568>`_
...
* Fix a race condition in coordinator query cleanup causing a task memory leak over time. `#27597 <https://github.com/prestodb/presto/pull/27597>`_
* Fix intermittent crashes in native execution caused by non-deterministic iteration order in certain optimizer rules that rewrite aggregation queries. `#27493 <https://github.com/prestodb/presto/pull/27493>`_
* Improve the ``GROUP BY + LIMIT`` optimization to skip partition keys during limiting and to use scan limiting instead of timeouts for more predictable performance. `#27678 <https://github.com/prestodb/presto/pull/27678>`_ `#27819 <https://github.com/prestodb/presto/pull/27819>`_
* Improve ``UNION ALL`` query performance by pruning empty subtrees at the connector PHYSICAL stage, eliminating idle operators when some branches are filtered out by partition or snapshot pruning. `#27765 <https://github.com/prestodb/presto/pull/27765>`_
...
* Improve ``map_from_entries(ARRAY[ROW(...), ...])`` performance by rewriting to ``MAP(ARRAY[keys], ARRAY[values])`` at plan time. `#27491 <https://github.com/prestodb/presto/pull/27491>`_
* Improve query planning performance for wide-column queries with faster field lookup and fast paths that skip unnecessary processing across multiple optimizer rules. `#27553 <https://github.com/prestodb/presto/pull/27553>`_ `#27547 <https://github.com/prestodb/presto/pull/27547>`_
* Add a limit guard (N <= 1000) to the ``GROUP BY + LIMIT`` optimization to prevent unexpected behavior on large limits. `#27678 <https://github.com/prestodb/presto/pull/27678>`_
* Add ``ALTER MATERIALIZED VIEW <name> SET PROPERTIES (...)`` SQL statement to update materialized view properties after creation. See :ref:`connector/iceberg:alter materialized view`. `#27806 <https://github.com/prestodb/presto/pull/27806>`_
...
* Add :ref:`admin/properties-session:\`\`partition_aware_grouped_execution\`\`` session property to schedule each (bucket, partition) combination as a separate lifespan in grouped execution, reducing per-lifespan data volumes for bucketed tables. Disabled by default. `#27663 <https://github.com/prestodb/presto/pull/27663>`_
* Add incremental refresh for materialized views, allowing the connector to refresh only changed data instead of recomputing the entire view. See :ref:`connector/iceberg:materialized views`. `#26959 <https://github.com/prestodb/presto/pull/26959>`_
* Add session property :ref:`admin/properties-session:\`\`join_prefilter_build_side_with_complex_probe_side\`\`` (default false) to extend join prefilter optimization to support complex probe-side patterns including ``UNION ALL``, cross joins, ``UNNEST``, and aggregation. `#27598 <https://github.com/prestodb/presto/pull/27598>`_
* Add optimizer that rewrites semi-joins into left joins with distinct aggregation when both sides are bucketed on the join key, avoiding a data shuffle. Gated behind session property :ref:`admin/properties-session:\`\`rewrite_bucketed_semi_join_to_join\`\`` (default disabled). `#27510 <https://github.com/prestodb/presto/pull/27510>`_
* Add optimizer that rewrites ``ROW IN`` predicates into ``OR`` of ``AND`` equality chains when all ``ROW`` fields are partition keys, enabling per-column partition pruning. Gated behind session property ``rewrite_row_constructor_in_to_disjunction`` (default disabled). `#27500 <https://github.com/prestodb/presto/pull/27500>`_
...
* Add support for Thrift serialization (``application/x-thrift-binary``, ``application/x-thrift-compact``, ``application/x-thrift-fb-compact``) to all TaskResource endpoints for faster internal communication. `#27486 <https://github.com/prestodb/presto/pull/27486>`_
* Add support for ``ALTER TABLE ... ALTER COLUMN ... SET DEFAULT`` syntax to update Iceberg column write-default values (requires Iceberg format version 3+). See :ref:`connector/iceberg:alter table`. `#27810 <https://github.com/prestodb/presto/pull/27810>`_
* Improve materialized view query rewriting to support ``GROUP BY`` and ``ORDER BY`` ordinal references (e.g., ``SELECT a, SUM(b) FROM t GROUP BY 1``). Previously, these queries would silently skip materialized view optimization. `#27422 <https://github.com/prestodb/presto/pull/27422>`_
* Improve materialized view query rewriting to support scalar functions (``CONCAT``, ``ABS``, ``JSON_EXTRACT``, ``CAST``, ``IF``, ``COALESCE``, ``CASE``). Queries using these functions now correctly rewrite to scan the materialized view. `#27549 <https://github.com/prestodb/presto/pull/27549>`_
...
* Expand the optimizer that rewrites ``ROW IN`` predicates to disjunctions to fire for all columns, not just partition keys, enabling better predicate pushdown. Gated behind session property ``rewrite_row_constructor_in_to_disjunction``. `#27680 <https://github.com/prestodb/presto/pull/27680>`_
* Add TopN late materialization optimization that sorts only the ``ORDER BY`` columns plus a unique ``$row_id`` first, then fetches full rows via a semi-join. Improves performance for ``ORDER BY ... LIMIT`` on wide tables. `#27641 <https://github.com/prestodb/presto/pull/27641>`_
* Fix a bug in ``StreamPropertyDerivations.visitTopN`` that could cause ``IllegalStateException`` during query planning. `#27664 <https://github.com/prestodb/presto/pull/27664>`_
* Add ``split_part_reverse`` as a global Presto SQL function, available in all queries without requiring native execution. `#27480 <https://github.com/prestodb/presto/pull/27480>`_
...
* Add ``iceberg.materialized-view-default-max-snapshots-per-refresh`` configuration property and matching session property to cap how far each base table can advance in a single ``REFRESH MATERIALIZED VIEW``. See :ref:`connector/iceberg:catalog configuration`. `#27774 <https://github.com/prestodb/presto/pull/27774>`_
* Add ``iceberg.materialized-view-default-storage-schema`` configuration property to route storage tables into a single schema. See :ref:`connector/iceberg:catalog configuration`. `#27728 <https://github.com/prestodb/presto/pull/27728>`_
* Add ``max_snapshots_per_refresh`` materialized view property to bound incremental refresh scope. Defaults to ``0`` (unbounded). Requires Iceberg V3 row lineage; V2 tables fall back to unbounded refresh. See :ref:`connector/iceberg:materialized view properties`. `#27774 <https://github.com/prestodb/presto/pull/27774>`_
* Add ``materialized_view_stitching_strategy`` and ``materialized_view_incremental_refresh_strategy`` session properties (values: `ALWAYS`, `NEVER`, `AUTOMATIC`; default: `ALWAYS`). Under `AUTOMATIC`, the optimizer selects between the rewrite and the full alternative based on cost. See :ref:`connector/iceberg:session properties`. `#27820 <https://github.com/prestodb/presto/pull/27820>`_
* Add support for Iceberg V3 initial default values when reading tables. `#27659 <https://github.com/prestodb/presto/pull/27659>`_
* Add incremental refresh for materialized views in the Iceberg connector. See :ref:`connector/iceberg:materialized views`. `#26959 <https://github.com/prestodb/presto/pull/26959>`_
* Add low and high values for ``VARCHAR``/``CHAR`` columns to Iceberg table statistics, enabling better query optimization. `#27357 <https://github.com/prestodb/presto/pull/27357>`_
* Add metastore cache invalidation procedure for the Iceberg connector. See :ref:`connector/iceberg:procedures`. `#27200 <https://github.com/prestodb/presto/pull/27200>`_
* Add predicate pushdown on ``_last_updated_sequence_number`` for file-level pruning during incremental refresh. `#27766 <https://github.com/prestodb/presto/pull/27766>`_
...
* Add support for ``min/max/count`` aggregation pushdown based on file stats. This can be toggled with the ``aggregate_push_down_enabled`` session property or the ``iceberg.aggregate-push-down-enabled`` configuration property. See :ref:`connector/iceberg:session properties` and :ref:`connector/iceberg:configuration properties`. `#27085 <https://github.com/prestodb/presto/pull/27085>`_
* Add support for ``ALTER TABLE ... ALTER COLUMN ... SET DATA TYPE`` in the Iceberg connector. See :ref:`connector/iceberg:alter table`. `#25418 <https://github.com/prestodb/presto/pull/25418>`_
